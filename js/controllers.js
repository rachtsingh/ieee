...
angular.module('ieee', ['ngRoute', 'ieee.controllers']);
...
angular.module('ieee.controllers', []).controller('SettingsCtrl', [function() {}]).controller('MainCtrl', ['$scope', function($scope) {
	...
	$scope.settings = {
		...
		weights: {
			...
			major: 5,
			company: 3,
			// the choice of the student matches
			fulltime: 4,
			/* Bonus given for seniors and grads matching full times*/
			intern: 4,
			/* Bonus given for sophomores and juniors matching interns*/
			repeat: 10 /* Penalty given to repeating the same assignment as before */
		}
	}
	...
	$scope.match = function(student, company) {
		...
		if (student.finalcompany == company) {
			weight -= $scope.settings.weights.repeat
		}
		return weight;
	}
			...
			for (var j = 0; j < $scope.companies.length; j++) {
				...
				try {
					for (var c = 0; c < $scope.companies[j].number; c++) {
						console.log("ADDING COMPANY", time, $scope.companies[j]);
						if (i == 0){
		...
		var env = {
			...
			show_answer: function show_answer() {
				...
				if (time == 1) {
					...
					for (var i = 0; i < $scope.students.length; i++) {
						$scope.students[i].secondcompany = $scope.map[$scope.answer[i]];
					}
				}
				$scope.$apply();
			}
